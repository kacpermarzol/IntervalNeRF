...
def train():
    ...
    if args.metrics_only:
        ...
        with torch.no_grad():
            for i in i_test[0:200]:
                ...
                rgb, _, _, _ = render(hh, ww, kk, chunk=args.chunk, c2w=pose,
                                      **render_kwargs_test)
    ...
    for i in trange(start, N_iters):
        ...
        rgb, disp, acc, extras = render(1, 1, 1, chunk=args.chunk, rays=batch_rays,
        ...
        optimizer.zero_grad()
        mask = mask.to(device)
        img_loss = img2mse2(rgb, target_s, mask)
        # trans = extras['raw'][...,-1]
        loss = img_loss
        ...
        if i % args.save_every == 0:
            ...
            with torch.no_grad():
                rgb, _, _, extras = render(1, 1, 1, chunk=args.chunk, rays=batch_rays_test,
        ...
        if i % args.i_video == 0 and i > 0:
            ...
            with torch.no_grad():
                rgbs800, disps800 = render_path(render_poses, hwf_800, K_800, args.chunk, render_kwargs_test)
                print('Done, saving', rgbs800.shape, disps800.shape)
                rgbs400, disps400 = render_path(render_poses, hwf_400, K_400, args.chunk, render_kwargs_test)
                print('Done, saving', rgbs400.shape, disps400.shape)
                rgbs200, disps200 = render_path(render_poses, hwf_200, K_200, args.chunk, render_kwargs_test)
                print('Done, saving', rgbs200.shape, disps200.shape)
                rgbs100, disps100 = render_path(render_poses, hwf_100, K_100, args.chunk, render_kwargs_test)
        ...
        if i % args.i_img == 0:
            ...
            with torch.no_grad():
                rgb, _, _, _ = \
                    render(hh, ww, kk, chunk=args.chunk, c2w=pose,
                           **render_kwargs_test)
